...
public interface AssessmentResultRepository extends JpaRepository<AssessmentResult, Long> {
    ...
    @Query("SELECT AVG(ar.score) FROM AssessmentResult ar WHERE ar.studentProfile.enrollmentId = :cohort AND ar.assessmentId LIKE CONCAT('%', :skillCode, '%')")
    Double avgScoreByCohortAndSkill(@Param("cohort") String cohort, @Param("skillCode") Long skillId);
}
